...
import type { ClassWithTeacherInfo, UserProfileWithId, Subject, ExamPeriod, ExamResult, School } from '@/types';
...
export default function AdminEnterResultsPage() {
  ...
  const fetchInitialData = useCallback(async () => {
    if (currentUser?.schoolId) {
      ...
      try {
        ...
        toast({title: "Error", description: "Could not load initial data for result entry.", variant: "destructive"});
      } finally {
        setIsLoadingData(false);
      }
    }
  }, [currentUser, getSchoolDetails, getClassesBySchool, getSubjectsBySchool, getExamPeriodsBySchool, toast]);
  ...
  const fetchStudentsAndExistingResults = useCallback(async () => {
    if (selectedClassId && selectedExamPeriodId && selectedSubjectId && currentUser?.schoolId) {
      setIsLoadingData(true);
      try {
        const [students, existingResultsForSubjectPeriodClass] = await Promise.all([
          getStudentsInClass(selectedClassId),
          getExamResultsByPeriodAndClass(selectedExamPeriodId, selectedClassId, currentUser.schoolId, selectedSubjectId)
        ]);
        ...
        students.forEach(s => {
          const existing = existingResultsForSubjectPeriodClass.find(er => er.studentId === s.id);
          initialResults[s.id] = { 
  ...
  }, [selectedClassId, selectedExamPeriodId, selectedSubjectId, currentUser?.schoolId, getStudentsInClass, getExamResultsByPeriodAndClass, toast]);
  ...
  const handleSubmitResults = async () => {
    if (!selectedExamPeriodId || !selectedClassId || !selectedSubjectId || Object.keys(results).length === 0 || !currentUser?.uid || !currentUser.schoolId || !currentUser.displayName) {
      toast({ title: "Missing Information", description: "Please select exam, class, subject, and ensure results are entered.", variant: "destructive" });
      return;
    }
    
    if (!schoolDetails?.isExamModeActive) {
        toast({ title: "Exam Mode Off", description: "School-wide Exam Mode is not active. Results cannot be entered. Please activate it in School Settings.", variant: "destructive" });
        return;
    }
    ...
    const className = allSchoolClasses.find(c => c.id === selectedClassId)?.name || "Selected Class";
        ...
        if (!success) {
            ...
            if (student && student.displayName) {
                await addNotification({
                    userId: student.id,
                    schoolId: currentUser.schoolId,
                    message: `Your results for ${subjectName} in ${examPeriodName} have been updated by an Administrator.`,
                ...
                if (parent && parent.schoolId === currentUser.schoolId) {
                     await addNotification({
                        userId: parent.id,
                        schoolId: currentUser.schoolId,
                        message: `${student.displayName}'s results for ${subjectName} in ${examPeriodName} have been updated by an Administrator.`,
                        type: 'exam_results_entered',
                        link: `/parent/results`,
                        actorName: 'School Administration'
                    });
    ...
    if (allSuccessful) {
        toast({ title: "Results Submitted!", description: `Results for ${examPeriodName} - ${className} - ${subjectName} have been recorded/updated by admin.` });
        if (currentUser.displayName && currentUser.schoolId) {
            await addActivity({
                ...
                classId: selectedClassId,
                type: 'exam_results_entered',
                message: `Admin ${currentUser.displayName} entered/updated results for ${examPeriodName} - ${className} - ${subjectName}.`,
            });
        }
    } else {
  ...
  const filteredClassesForSelectedPeriod = allSchoolClasses.filter(cls => 
    examPeriods.find(ep => ep.id === selectedExamPeriodId)?.assignedClassIds.includes(cls.id)
  );

  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold">Admin: Enter Exam Results</h1>
        ...
        <Card className="border-destructive bg-destructive/10">
            ...
            <CardContent>
                <p className="text-destructive-foreground">
                    School-wide Exam Mode is currently OFF. Teachers and admins cannot enter or edit exam results.
                    Please activate Exam Mode in <Link href="/admin/settings" className="underline hover:text-foreground">School Settings</Link> to proceed.
                </p>
            </CardContent>
        </Card>
      )}
      ...
      <Card className="card-shadow">
        <CardHeader>
          <CardTitle className="flex items-center"><FileEdit className="mr-2 h-5 w-5 text-primary"/>Select Exam Period, Class, and Subject</CardTitle>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <Label htmlFor="exam-period-select-admin">Exam Period</Label>
            <Select onValueChange={setSelectedExamPeriodId} value={selectedExamPeriodId} disabled={isLoadingSelectors || examPeriods.length === 0}>
              <SelectTrigger id="exam-period-select-admin"><SelectValue placeholder="Select exam..." /></SelectTrigger>
              <SelectContent>
                {examPeriods.map(ep => (
            ...
            {examPeriods.length === 0 && !isLoadingSelectors && <p className="text-xs text-muted-foreground mt-1">No active/grading exam periods available.</p>}
          </div>
          <div>
            <Label htmlFor="class-select-admin-exam">Class</Label>
            <Select onValueChange={setSelectedClassId} value={selectedClassId} disabled={isLoadingSelectors || !selectedExamPeriodId || filteredClassesForSelectedPeriod.length === 0}>
              <SelectTrigger id="class-select-admin-exam"><SelectValue placeholder="Select class..." /></SelectTrigger>
              <SelectContent>
                {filteredClassesForSelectedPeriod.map(cls => (
                  <SelectItem key={cls.id} value={cls.id}>{cls.name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            {!selectedExamPeriodId && <p className="text-xs text-muted-foreground mt-1">Select an exam period first.</p>}
            {selectedExamPeriodId && filteredClassesForSelectedPeriod.length === 0 && !isLoadingSelectors && <p className="text-xs text-muted-foreground mt-1">No classes assigned to this exam period.</p>}
          </div>
          <div>
            <Label htmlFor="subject-select-admin-exam">Subject</Label>
             <Select onValueChange={setSelectedSubjectId} value={selectedSubjectId} disabled={isLoadingSelectors || !selectedClassId || schoolSubjects.length === 0}>
              <SelectTrigger id="subject-select-admin-exam"><SelectValue placeholder="Select subject..." /></SelectTrigger>
              <SelectContent>
            ...
            {selectedClassId && schoolSubjects.length === 0 && !isLoadingSelectors && <p className="text-xs text-muted-foreground mt-1">No subjects configured for the school.</p>}
          </div>
        </CardContent>
      </Card>
      ...
      {selectedExamPeriodId && selectedClassId && selectedSubjectId && (
        <Card className="card-shadow">
          <CardHeader>
            ...
            <CardDescription>
              For: {examPeriods.find(ep => ep.id === selectedExamPeriodId)?.name} - {allSchoolClasses.find(c=>c.id===selectedClassId)?.name} - {schoolSubjects.find(s => s.id === selectedSubjectId)?.name}
            </CardDescription>
          </CardHeader>
          <CardContent>
